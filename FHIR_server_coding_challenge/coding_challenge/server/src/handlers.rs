...
pub async fn get_patient_history(
    ...
    match db.get_patient_history(&id).await {
        Ok(history) => {
            ...
            let bundle_last_updated = history
                .first()
                .map(|(_, ts, _, _)| ts.clone());
...
mod tests {
    ...
    async fn test_get_patient_handler() {
        ...
        let (_, _, created) = create_patient(State(db.clone()), Json(patient)).await.unwrap();
    ...
    async fn test_search_patients_handler() {
        ...
        let _ = create_patient(State(db.clone()), Json(patient1)).await.unwrap();
        let _ = create_patient(State(db.clone()), Json(patient2)).await.unwrap();
    ...
    async fn test_search_patients_by_gender_handler() {
        ...
        let _ = create_patient(State(db.clone()), Json(patient)).await.unwrap();
        ...
        assert!(!bundle.entry.is_empty());
        assert!(bundle.entry.iter().all(|e| e.resource.gender == Some("other".to_string())));
    }
