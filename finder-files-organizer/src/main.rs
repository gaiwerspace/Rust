...
#[command(version)]
struct Args {
    /// Directory to open and sort (not required in tray mode)
...
fn main() -> Result<(), String> {
    ...
    if args.tray {
        let tray_app = tray_app::TrayApp::new();
        return tray_app.run().map_err(|e| e.to_string());
    }

    // Validate thread pool configuration first
    let config = ThreadPoolConfig::from_args(&args)?;

    // Validate that path is provided when not in tray mode
    let path = args.path.ok_or_else(|| "Error: PATH argument is required when not using --tray mode. Usage: finder-files-organizer <PATH> [OPTIONS]".to_string())?;
    ...
    if args.pack_to_folders {
        ...
        let (moved, skipped) = if args.recursive {
            ...
            organizer.organize_recursive_with_threads(&path, config.thread_count)?
        } else {
            organizer.organize_with_threads(&path, config.thread_count)?
        };
        ...
        if args.sort.is_some() || args.order.is_some() {
            ...
            if args.recursive {
                sorter.sort_recursively(&path, sort_by, order)?;
            } else {
                sorter.sort_finder_window(&path, sort_by, order)?;
            }

            eprintln!("Sorting completed!");
        }
